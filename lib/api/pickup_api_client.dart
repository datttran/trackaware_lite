...
class PickUpApiClient {
  final http.Client httpClient;

  PickUpApiClient({@required this.httpClient}) : assert(httpClient != null);
  ...
  Future<List<PackageDetailsResponse>> verifyDestination(String tag) async {
    final packageDetailsUrl = globals.baseUrl + "/packagedetails/";

    String basicAuth = 'Basic ' +
        base64Encode(
            utf8.encode(globals.serverUserName + ':' + globals.serverPassword));

    var packageDetailsResponse = await this.httpClient.post(packageDetailsUrl,
        headers: {'authorization': basicAuth}, body: tag);
    final packageDetailsResponseJson =
        jsonDecode(packageDetailsResponse.body) as List;
    print("PackageDetailsResponse :" + packageDetailsResponseJson.toString());
    return packageDetailsResponseJson
        .map((data) => new PackageDetailsResponse.fromJson(data))
        .toList();
  }
