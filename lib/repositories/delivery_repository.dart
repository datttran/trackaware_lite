...
class DeliveryApiRepository {
  ...
  DeliveryApiRepository({@required this.deliveryApiClient})
  ...
  Future<List<LocationResponse>> fetchLocation() async {
    try {
      ...
      if (list.isNotEmpty) {
        ...
        return await deliveryApiClient.fetchLocation();
      }
    } catch (error) {
      print(error);
      return await deliveryApiClient.fetchLocation();
  ...
  Future<DeliveryResponse> updateDeliveryStatus(
    ...
    return await deliveryApiClient.updateDeliveryStatus(deliveryRequest);
  }
